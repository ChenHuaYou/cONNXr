...
int test_operator(char *outputName)
{
    ...
    Onnx__TensorProto **inputs = malloc(10*sizeof(Onnx__TensorProto*));
    glob_t globbuf;
    int nInputs = 0;
    if (0==glob(inputPath, 0, NULL, &globbuf)){
        char **inputPbs=globbuf.gl_pathv;
        for(;*inputPbs;inputPbs++){
            TRACE_LEVEL0("Reading input %s ...\n", *inputPbs);
    ...
    TRACE_LEVEL0("%s:\n",model->graph->node[0]->outputs[0]->name);
    int result = compareAlmostEqualTensorProto(model->graph->node[0]->outputs[0], out0set0);
    TRACE_LEVEL0("Result %d\n", result);

    //free inputs
    for(int i=0; i<nInputs; i++){
        freeTensorData(inputs[i]);
    }
    freeTensorData(out0set0);
...
double test_model(
    ...
    TRACE_LEVEL0("Will compare output %d = %s\n", (int)model->graph->n_node, model->graph->node[model->graph->n_node-1]->outputs[0]->name);
    int exit_status = compareAlmostEqualTensorProto(model->graph->node[model->graph->n_node-1]->outputs[0], out0);
