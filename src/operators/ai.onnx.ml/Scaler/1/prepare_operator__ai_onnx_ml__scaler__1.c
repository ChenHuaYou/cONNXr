//this file was generated by ../../../../../../scripts/onnx_generator/OperatorTemplate.py
#include "operator__ai_onnx_ml__scaler__1.h"
#include "tracing.h"
#include "utils.h"

operator_status
prepare_operator__ai_onnx_ml__scaler__1(
    node_context *ctx
)
{
    TRACE_ENTRY(1);

    TRACE_NODE(2, true, ctx->onnx_node);

    /* UNCOMMENT AS NEEDED */

    // Onnx__TensorProto *i_X = searchInputByName(ctx, 0);

    // TRACE_TENSOR(2, true, i_X);

    // Onnx__AttributeProto *a_offset = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"offset");
    // Onnx__AttributeProto *a_scale = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"scale");

    // TRACE_ATTRIBUTE(2, a_offset, a_offset);
    // TRACE_ATTRIBUTE(2, a_scale, a_scale);

    // Onnx__TensorProto *o_Y = searchOutputByName(ctx, 0);

    /* ALLOCATE AND INITIALIZE CONTEXT HERE IF NEEDED */

    // size_t default_n_offset = ;
    // float* default_offset = ;
    // size_t default_n_scale = ;
    // float* default_scale = ;

    // context_operator__ai_onnx_ml__scaler__1 *op_ctx = NULL;
    // op_ctx = malloc(sizeof(context_operator__ai_onnx_ml__scaler__1));
    // TRACE_FATAL(0 , !op_ctx, "could not allocate executer_context");

    // op_ctx->n_offset = a_offset?a_offset->n_floats:default_n_offset;
    // op_ctx->offset = a_offset?a_offset->floats:ARRAYDUP(default_offset,default_n_offset);
    // TRACE_FATAL(0, !op_ctx->offset, "malloc failed");
    // op_ctx->n_scale = a_scale?a_scale->n_floats:default_n_scale;
    // op_ctx->scale = a_scale?a_scale->floats:ARRAYDUP(default_scale,default_n_scale);
    // TRACE_FATAL(0, !op_ctx->scale, "malloc failed");

    // TRACE_ARRAY(2, true, op_ctx->offset, , op_ctx->n_offset, "%f");
    // TRACE_ARRAY(2, true, op_ctx->scale, , op_ctx->n_scale, "%f");

    /* INITIALIZE OUTPUTS DATA_TYPE AND SHAPE HERE */


    /* MALLOC OUTPUT TENSORS HERE */

    // mallocTensorData(o_Y);

    // TRACE_TENSOR(2, true, o_Y);

    /* CHOOSE EXECUTER AND CONTEXT HERE */
    /* YOU MAY USE THE GENERATED RESOLVER */

    // ctx->executer = resolve_operator__ai_onnx_ml__scaler__1(ctx);
    // ctx->executer_context = op_ctx;

    TRACE_EXIT(1);

    /* CHANGE RETURN CODE IF THIS PREPARER IS VALID */
    return OP_ENOSYS;
    // return OP_OK;
}