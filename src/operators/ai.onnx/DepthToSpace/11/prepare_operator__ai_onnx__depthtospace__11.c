...
)
{
    ...
    // Onnx__AttributeProto *a_blocksize = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"blocksize");
    // Onnx__AttributeProto *a_mode = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"mode");

    // TRACE_ATTRIBUTE(2, true, a_blocksize);
    ...
    // context_operator__ai_onnx__depthtospace__11 *op_ctx = NULL;
    // op_ctx = malloc(sizeof(context_operator__ai_onnx__depthtospace__11));
    // TRACE_FATAL(0 , !op_ctx, "could not allocate executer_context");

    // op_ctx->blocksize = a_blocksize->i;
    // op_ctx->mode = a_mode?strndup((char*)a_mode->s.data, a_mode->s.len):default_mode;

    // TRACE_VAR(2, true, op_ctx->blocksize, "%" PRId64);
    ...
    // ctx->executer = resolve_operator__ai_onnx__depthtospace__11(ctx);
