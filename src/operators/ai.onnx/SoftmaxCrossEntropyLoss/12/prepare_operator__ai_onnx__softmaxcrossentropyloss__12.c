//this file was generated by ../../../../../../scripts/onnx_generator/OperatorTemplate.py
#include "operator__ai_onnx__softmaxcrossentropyloss__12.h"
#include "tracing.h"
#include "utils.h"

operator_status
prepare_operator__ai_onnx__softmaxcrossentropyloss__12(
    Onnx__NodeProto *ctx
)
{
    TRACE_ENTRY(1);

    TRACE_NODE(2, true, ctx->onnx_node);

    /* UNCOMMENT AS NEEDED */

    //Onnx__TensorProto *i_scores = searchInputByIndex(ctx, 0);
    //Onnx__TensorProto *i_labels = searchInputByIndex(ctx, 1);
    //Onnx__TensorProto *i_weights = searchInputByIndex(ctx, 2);

    // TRACE_TENSOR(2, true, i_scores);
    // TRACE_TENSOR(2, true, i_labels);
    // TRACE_TENSOR(2, weights, i_weights);

    // Onnx__AttributeProto *a_ignore_index = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"ignore_index");
    // Onnx__AttributeProto *a_reduction = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"reduction");

    // TRACE_ATTRIBUTE(2, a_ignore_index, a_ignore_index);
    // TRACE_ATTRIBUTE(2, a_reduction, a_reduction);

    Onnx__TensorProto *o_output = searchOutputByIndex(ctx, 0);
    Onnx__TensorProto *o_log_prob = searchOutputByIndex(ctx, 1);

    /* ALLOCATE AND INITIALIZE CONTEXT HERE IF NEEDED */

    // int64_t default_ignore_index = ;
    // char* default_reduction = ;

    // context_operator__ai_onnx__softmaxcrossentropyloss__12 *op_ctx = NULL;
    // op_ctx = malloc(sizeof(context_operator__ai_onnx__softmaxcrossentropyloss__12));
    // TRACE_FATAL(0 , !op_ctx, "could not allocate executer_context");

    // op_ctx->ignore_index = a_ignore_index?a_ignore_index->i:default_ignore_index;
    // op_ctx->reduction = a_reduction?strndup((char*)a_reduction->s.data, a_reduction->s.len):default_reduction;

    // TRACE_VAR(2, true, op_ctx->ignore_index, "%" PRId64);
    // TRACE_VAR(2, true, op_ctx->reduction, "\"%s\"");

    /* INITIALIZE OUTPUTS DATA_TYPE AND SHAPE HERE */


    /* MALLOC OUTPUT TENSORS HERE */

    mallocTensorData(o_output);
    mallocTensorData(o_log_prob);

    // TRACE_TENSOR(2, true, o_output);
    // TRACE_TENSOR(2, log_prob, o_log_prob);

    /* CHOOSE EXECUTER AND CONTEXT HERE */
    /* YOU MAY USE THE GENERATED RESOLVER */

    ctx->executer = execute_operator__ai_onnx__softmaxcrossentropyloss__12;
    // ctx->executer_context = op_ctx;

    TRACE_EXIT(1);

    /* CHANGE RETURN CODE IF THIS PREPARER IS VALID */
    return OP_ENOSYS;
    // return OP_OK;
}