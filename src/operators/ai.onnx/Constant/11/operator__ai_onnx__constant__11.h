//this file was generated by ../../../../../../scripts/onnx_generator/OperatorHeader.py
# ifndef OPERATOR_OPERATOR__AI_ONNX__CONSTANT__11_H
# define OPERATOR_OPERATOR__AI_ONNX__CONSTANT__11_H

# include "operators/operator.h"
# include "operators/operator_stub.h"
# include "operators/operator_info.h"

/**
 * ai.onnx operator 'Constant' version 11
 *
 * @param[in]  ctx  Operator context
 * @return          Status code
 *
 * A constant tensor. Exactly one of the two attributes, either value or sparse_value,
 * must be specified.
 * 
 * Constraint T:
 *   Constrain input and output types to all tensor types.
 *   Allowed Types: tensor_bool, tensor_complex128, tensor_complex64,
 *                  tensor_double, tensor_float, tensor_float16, tensor_int16,
 *                  tensor_int32, tensor_int64, tensor_int8, tensor_string,
 *                  tensor_uint16, tensor_uint32, tensor_uint64, tensor_uint8

 * Output T output:
 *   Output tensor containing the same value of the provided tensor.
 *   Allowed Types: tensor_bool, tensor_complex128, tensor_complex64,
 *                  tensor_double, tensor_float, tensor_float16, tensor_int16,
 *                  tensor_int32, tensor_int64, tensor_int8, tensor_string,
 *                  tensor_uint16, tensor_uint32, tensor_uint64, tensor_uint8
 * Attribute SPARSE_TENSOR sparse_value (optional):
 *   The value for the elements of the output tensor in sparse format.
 * 
 * Attribute TENSOR value (optional):
 *   The value for the elements of the output tensor.
 *
 * @since version 11
 *
 * @see io/onnx/onnx/defs/generator/old.cc:128
 * @see https://github.com/onnx/onnx/blob/master/docs/Operators.md#Constant
 */

operator_status
prepare_operator__ai_onnx__constant__11(
    node_context *ctx
);

extern operator_info info_operator__ai_onnx__constant__11;

typedef struct {
    Onnx__SparseTensorProto* sparse_value;
    Onnx__TensorProto* value;

} context_operator__ai_onnx__constant__11;

operator_executer
resolve_operator__ai_onnx__constant__11(
    node_context *ctx
);

operator_status
execute_operator__ai_onnx__constant__11(
    node_context *ctx
);

# endif