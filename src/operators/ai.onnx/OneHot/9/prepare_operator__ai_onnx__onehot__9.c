//this file was generated by ../../../../../../scripts/onnx_generator/OperatorTemplate.py
#include "operator__ai_onnx__onehot__9.h"
#include "tracing.h"
#include "utils.h"

operator_status
prepare_operator__ai_onnx__onehot__9(
    Onnx__NodeProto *ctx
)
{
    TRACE_ENTRY(1);

    TRACE_NODE(2, true, ctx->onnx_node);

    /* UNCOMMENT AS NEEDED */

    //Onnx__TensorProto *i_indices = searchInputByIndex(ctx, 0);
    //Onnx__TensorProto *i_depth = searchInputByIndex(ctx, 1);
    //Onnx__TensorProto *i_values = searchInputByIndex(ctx, 2);

    // TRACE_TENSOR(2, true, i_indices);
    // TRACE_TENSOR(2, true, i_depth);
    // TRACE_TENSOR(2, true, i_values);

    // Onnx__AttributeProto *a_axis = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"axis");

    // TRACE_ATTRIBUTE(2, a_axis, a_axis);

    Onnx__TensorProto *o_output = searchOutputByIndex(ctx, 0);

    /* ALLOCATE AND INITIALIZE CONTEXT HERE IF NEEDED */

    // int64_t default_axis = ;

    // context_operator__ai_onnx__onehot__9 *op_ctx = NULL;
    // op_ctx = malloc(sizeof(context_operator__ai_onnx__onehot__9));
    // TRACE_FATAL(0 , !op_ctx, "could not allocate executer_context");

    // op_ctx->axis = a_axis?a_axis->i:default_axis;

    // TRACE_VAR(2, true, op_ctx->axis, "%" PRId64);

    /* INITIALIZE OUTPUTS DATA_TYPE AND SHAPE HERE */


    /* MALLOC OUTPUT TENSORS HERE */

    mallocTensorData(o_output);

    // TRACE_TENSOR(2, true, o_output);

    /* CHOOSE EXECUTER AND CONTEXT HERE */
    /* YOU MAY USE THE GENERATED RESOLVER */

    ctx->executer = execute_operator__ai_onnx__onehot__9;
    // ctx->executer_context = op_ctx;

    TRACE_EXIT(1);

    /* CHANGE RETURN CODE IF THIS PREPARER IS VALID */
    return OP_ENOSYS;
    // return OP_OK;
}