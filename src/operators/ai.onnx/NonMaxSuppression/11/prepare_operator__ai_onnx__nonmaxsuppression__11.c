...
)
{
    ...
    //Onnx__TensorProto *i_boxes = searchInputByIndex(ctx, 0);
    //Onnx__TensorProto *i_scores = searchInputByIndex(ctx, 1);
    //Onnx__TensorProto *i_max_output_boxes_per_class = searchInputByIndex(ctx, 2);
    //Onnx__TensorProto *i_iou_threshold = searchInputByIndex(ctx, 3);
    //Onnx__TensorProto *i_score_threshold = searchInputByIndex(ctx, 4);
    ...
    Onnx__TensorProto *o_selected_indices = searchOutputByIndex(ctx, 0);

    /* ALLOCATE AND INITIALIZE CONTEXT HERE IF NEEDED */

    // int64_t default_center_point_box = ;

    // context_operator__ai_onnx__nonmaxsuppression__11 *op_ctx = NULL;
    // op_ctx = malloc(sizeof(context_operator__ai_onnx__nonmaxsuppression__11));
    // TRACE_FATAL(0 , !op_ctx, "could not allocate executer_context");

    // op_ctx->center_point_box = a_center_point_box?a_center_point_box->i:default_center_point_box;
    ...
    ctx->executer = execute_operator__ai_onnx__nonmaxsuppression__11;
