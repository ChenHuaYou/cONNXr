//this file was generated by ../../../../../../scripts/onnx_generator/OperatorTemplate.py
#include "operator__ai_onnx__and__1.h"
#include "tracing.h"
#include "utils.h"

operator_status
prepare_operator__ai_onnx__and__1(
    node_context *ctx
)
{
    TRACE_ENTRY(1);

    TRACE_NODE(2, true, ctx->onnx_node);

    /* UNCOMMENT AS NEEDED */

    // Onnx__TensorProto *i_A = searchInputByName(ctx, 0);
    // Onnx__TensorProto *i_B = searchInputByName(ctx, 1);

    // TRACE_TENSOR(2, true, i_A);
    // TRACE_TENSOR(2, true, i_B);

    // Onnx__AttributeProto *a_axis = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"axis");
    // Onnx__AttributeProto *a_broadcast = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"broadcast");

    // TRACE_ATTRIBUTE(2, a_axis, a_axis);
    // TRACE_ATTRIBUTE(2, a_broadcast, a_broadcast);

    // Onnx__TensorProto *o_C = searchOutputByName(ctx, 0);

    /* ALLOCATE AND INITIALIZE CONTEXT HERE IF NEEDED */

    // int64_t default_axis = ;
    // int64_t default_broadcast = ;

    // context_operator__ai_onnx__and__1 *op_ctx = NULL;
    // op_ctx = malloc(sizeof(context_operator__ai_onnx__and__1));
    // TRACE_FATAL(0 , !op_ctx, "could not allocate executer_context");

    // op_ctx->axis = a_axis?a_axis->i:default_axis;
    // op_ctx->broadcast = a_broadcast?a_broadcast->i:default_broadcast;

    // TRACE_VAR(2, true, op_ctx->axis, "%" PRId64);
    // TRACE_VAR(2, true, op_ctx->broadcast, "%" PRId64);

    /* INITIALIZE OUTPUTS DATA_TYPE AND SHAPE HERE */


    /* MALLOC OUTPUT TENSORS HERE */

    // mallocTensorData(o_C);

    // TRACE_TENSOR(2, true, o_C);

    /* CHOOSE EXECUTER AND CONTEXT HERE */
    /* YOU MAY USE THE GENERATED RESOLVER */

    // ctx->executer = resolve_operator__ai_onnx__and__1(ctx);
    // ctx->executer_context = op_ctx;

    TRACE_EXIT(1);

    /* CHANGE RETURN CODE IF THIS PREPARER IS VALID */
    return OP_ENOSYS;
    // return OP_OK;
}