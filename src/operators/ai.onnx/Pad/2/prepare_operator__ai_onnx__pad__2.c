//this file was generated by ../../../../../../scripts/onnx_generator/OperatorTemplate.py
#include "operator__ai_onnx__pad__2.h"
#include "tracing.h"
#include "utils.h"

operator_status
prepare_operator__ai_onnx__pad__2(
    node_context *ctx
)
{
    TRACE_ENTRY(1);

    TRACE_NODE(2, true, ctx->onnx_node);

    /* UNCOMMENT AS NEEDED */

    // Onnx__TensorProto *i_data = searchInputByName(ctx, 0);

    // TRACE_TENSOR(2, true, i_data);

    // Onnx__AttributeProto *a_mode = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"mode");
    // Onnx__AttributeProto *a_pads = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"pads");
    // Onnx__AttributeProto *a_value = searchAttributeNyName(ctx->onnx_node->n_attribute,ctx->onnx_node->attribute,"value");

    // TRACE_ATTRIBUTE(2, a_mode, a_mode);
    // TRACE_ATTRIBUTE(2, true, a_pads);
    // TRACE_ATTRIBUTE(2, a_value, a_value);

    // Onnx__TensorProto *o_output = searchOutputByName(ctx, 0);

    /* ALLOCATE AND INITIALIZE CONTEXT HERE IF NEEDED */

    // char* default_mode = ;
    // float default_value = ;

    // context_operator__ai_onnx__pad__2 *op_ctx = NULL;
    // op_ctx = malloc(sizeof(context_operator__ai_onnx__pad__2));
    // TRACE_FATAL(0 , !op_ctx, "could not allocate executer_context");

    // op_ctx->mode = a_mode?strndup((char*)a_mode->s.data, a_mode->s.len):default_mode;
    // op_ctx->n_pads = a_pads->n_ints;
    // op_ctx->pads = a_pads->ints;
    // op_ctx->value = a_value?a_value->f:default_value;

    // TRACE_VAR(2, true, op_ctx->mode, "\"%s\"");
    // TRACE_ARRAY(2, true, op_ctx->pads, , op_ctx->n_pads, "%" PRId64);
    // TRACE_VAR(2, true, op_ctx->value, "%f");

    /* INITIALIZE OUTPUTS DATA_TYPE AND SHAPE HERE */


    /* MALLOC OUTPUT TENSORS HERE */

    // mallocTensorData(o_output);

    // TRACE_TENSOR(2, true, o_output);

    /* CHOOSE EXECUTER AND CONTEXT HERE */
    /* YOU MAY USE THE GENERATED RESOLVER */

    // ctx->executer = resolve_operator__ai_onnx__pad__2(ctx);
    // ctx->executer_context = op_ctx;

    TRACE_EXIT(1);

    /* CHANGE RETURN CODE IF THIS PREPARER IS VALID */
    return OP_ENOSYS;
    // return OP_OK;
}